...
contract FlightSuretyData {
    ...
    mapping(uint => address) passengerAddresses;
    uint256 public passengerID = 0;
    ...
    */
                            ...
                            returns (bool)
    {
        ...
        require(airlines[registrar].funded > 0, "Only funded airlines can register new airlines");
        ...
        if(airlinesCount < MULTIPARTY_MIN_AIRLINES){
            ...
            require(airlines[airlineAddress].votes >= airlinesCount.div(2), "Atleast 50% of the funded airlines have to vote for this airline's registration");
            airlines[airlineAddress] = Airline({
                                                airlineWallet: airlineAddress,
                                                isRegistered: true,
                                                name: name,
    ...
    function voteAirline(address candidate) public requireIsOperational returns(bool) {
        require(airlines[msg.sender].funded > 0, "Only funded airlines can vote");
        airlines[candidate].votes = airlines[candidate].votes.add(1);
        return true;
    ...
    */
    ...
    function checkIfContains(address passenger) internal view returns(bool inList){
        ...
        for(uint256 c = 0; c <= passengerID; c++){
    ...
    function checkPassenger( address passAddr) external view returns(address found){
        for(uint256 ID = 0; ID <= passengerID; ID++){
            if(passengerAddresses[ID] == passAddr )
            {found = passAddr;}
        }
        return found;
    }
                                ...
                                requireIsOperational
    {
        //for (uint256 c = 0; c < passengerAddresses.length; c++) {
            ...
            if(passengers[passengerAddresses[c]].boughtFlight[flightCode] != 0) {
                ...
                passengers[passengerAddresses[c]].credit = savedCredit + payedPrice.mul(3).div(2);
            }
        }
    }
    ...
    */
    function fund
                            ...
                            requireIsOperational
    {
        require(msg.value>= MINIMUM_FUNDS, "Note: the minimum amount of funding is 10 ether");
